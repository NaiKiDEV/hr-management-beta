...
const UserProfileProvider = ({ children }: PropsWithChildren) => {
  ...
  }, [sessionData, userProfile, handleAccountSync, authCheckPassed]);
  ...
  if (!authCheckPassed && !error) {
    return (
      <div className="flex flex-grow flex-col items-center justify-center gap-6">
        <div className="text-center">
          <div role="status">
            <svg
              className="mr-2 inline h-16 w-16 animate-spin fill-emerald-400 text-gray-200 dark:text-slate-600"
              viewBox="0 0 100 101"
              fill="none"
  ...
  return sessionData && !error ? (
    ...
    <div className="flex flex-grow flex-col items-center justify-center gap-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-center text-3xl text-white">HR Management</h1>
        {error && (
          <div className="rounded-2xl bg-red-600 px-4 py-2">
            <p className="text-sm text-white">An error occurred: {error}</p>
          </div>
        )}
        <p className="text-center text-sm text-slate-300">
          This application supports SSO sign in, login with your workspace
          credentials to continue.
        </p>
      </div>
      {!sessionData ? (
        <button
          className="rounded-full bg-teal-600 px-4 py-2 text-sm font-semibold text-white no-underline transition hover:bg-teal-500"
          onClick={handleLogin}
        >
          Sign in through SSO
        </button>
